:Namespace NugetConsum
    ...
    :Class Package
     ...
     ⍝ Baas: avoid crashes if netstandard not found in path (as with chromedriver)
          std←sort1'netstandard'{⍵,⍨(⍺ startswith ⍵)/⍵}fws
          r←⊃std
        ∇
        ...
        ∇ r←CreateDotnetProject;path;p
          :If projectDir≡⍬
              ...
              :If ~⎕NEXISTS projectDir
                  ⎕CMD'mkdir ',projectDir
              :EndIf
          :EndIf
          r←projectDir
          ⎕CMD'dotnet new classlib -o ',projectDir
         
          :For p :In Packages
              ⎕CMD'dotnet add ',projectDir,' package ',p.addPkgStr
          :EndFor
        ∇
